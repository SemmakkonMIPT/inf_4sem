...
void PlayerState::double_jump(Player* player)
{
    player->mPosition.y -= 1;
    player->mVelocity.y = -kDoubleJumpingVelocity;
    startFalling(player);
}
...
Sitting::Sitting(Player* player) : PlayerState()
{
    ...
    player->mCollisionRect = player->mScaleFactor * sf::FloatRect(-10, -7, 20, 22);; //-10, -15, 20, 30
...
FirstAttack::FirstAttack(Player* player) : PlayerState()
{
    ...
    mAnimation.addTextureRect({ 53, 224, 50, 38});
    mAnimation.addTextureRect({ 103, 222, 50, 41});
    mAnimation.addTextureRect({ 153, 222, 50, 42});
    mAnimation.addTextureRect({ 203, 224, 50, 38});
    mAnimation.addTextureRect({ 253, 224, 50, 38});
    cout << "Creating FirstAttack state" << endl;
}

std::vector<float> FirstAttack::getCircleParameters() const {
    return {15, -15, 65};
}
...
void FirstAttack::update(Player* player, float dt)
{
     ...
     sf::Vector2f velocity = player->mVelocity;

   if (velocity != sf::Vector2f(0.0f, 0.0f))
    {
        sf::Vector2f deceleration = -kAcceleration * velocity / std::sqrt(velocity.x * velocity.x + velocity.y * velocity.y);
    ...
    mAnimation.update(dt);
    //if (mAnimation.getTime() + 2 * dt > mAnimation.getAnimationSize()/mAnimation.getAnimationSpeed())
    if (mAnimation.isAnimationEnded())
    {
...
SecondAttack::SecondAttack(Player* player) : PlayerState()
{
    mAnimation = Animation(Animation::AnimationType::OneIteration);
    mAnimation.setAnimationSpeed(10);
    mAnimation.addTextureRect({ 2, 260, 50, 40});
    mAnimation.addTextureRect({ 52, 260, 50, 40});
    mAnimation.addTextureRect({ 102, 260, 50, 40});
    mAnimation.addTextureRect({ 152, 260, 50, 40});
    
    cout << "Creating SecondAttack state" << endl;
}

std::vector<float> SecondAttack::getCircleParameters() const {
    return {25, 0, 60};
}
...
ThirdAttack::ThirdAttack(Player* player) : PlayerState()
{
    mAnimation = Animation(Animation::AnimationType::OneIteration);
    mAnimation.setAnimationSpeed(10);
    mAnimation.addTextureRect({ 150, 260, 50, 40});
    mAnimation.addTextureRect({ 200, 260, 50, 40});
    mAnimation.addTextureRect({ 250, 260, 50, 40});
    mAnimation.addTextureRect({ 300, 260, 50, 40});
    mAnimation.addTextureRect({ 0, 297, 50, 40});
    mAnimation.addTextureRect({ 50, 297, 50, 40});
    mAnimation.addTextureRect({ 100, 297, 50, 40});
...
std::vector<float> ThirdAttack::getCircleParameters() const {
    return {55, 15, 45};
}
...
{
    if (!isDoubleJumped && event.type == sf::Event::KeyPressed)
    {
        if (event.key.code == sf::Keyboard::Space)
        {
            double_jump(player);
