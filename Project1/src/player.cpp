...
CircleObject Player::getPlayerCircle()
{
    std::vector<float> Parameters = mpState->getCircleParameters();
    if (mIsFacedRight)
    {
        sf::Vector2f circleCenter = mPosition + sf::Vector2f(Parameters[0], Parameters[1]);
        return CircleObject(circleCenter, Parameters[2]);
    }
    else
    {
        sf::Vector2f circleCenter = mPosition + sf::Vector2f(-Parameters[0], Parameters[1]);
...
{
    return (this->mpState->isAttacking());
}
...
void Player::handleAllCollisions(const std::vector<sf::FloatRect>& blocks, const std::vector<CircleObject>& circles)
{
    mIsColliding = false;
    ...
    for (const CircleObject& circleObject : circles)
    {
        sf::FloatRect collisionRect(circleObject.center.x - circleObject.radius,
                                     circleObject.center.y - circleObject.radius,
                                     2 * circleObject.radius,
                                     2 * circleObject.radius);
        if (handleCollision(collisionRect))
            mIsColliding = true;
    }
